...
Install_qbittorrent()
{
	...
	if [ ! -f  $serverPath/lib/libtorrent ];then
		...
		./configure --prefix=$serverPath/lib/libtorrent --disable-gui CXXFLAGS=-std=c++11 && make && make install && make clean
	...
	cd ${QB_DIR}/qBittorrent-release-4.1.5 && ./configure --prefix=$serverPath/qbittorrent --disable-gui  CXXFLAGS=-std=c++11 && make && make install && make clean
